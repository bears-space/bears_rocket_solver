...
`packages/` and reads inputs from `inputs/`

`inputs/`: The inputs directory. The end goal of the project is to have this
directory be the input interface, with the solver program reading only from JSON
files in this directory without having to modify the `solver.py` script itself

`figures/`: Stores diagrams generated by the solver script
...
`packages/BEARS_Atmo/`: The atmospheric model. Currently uses the
...
MDAO organizes the components into a hierarchical tree-like [class
structure](https://openmdao.org/newdocs/versions/latest/theory_manual/class_structure.html).
...
| --------- | -------------- | ------ | ------- | -------- |
| ModelGroup | Group | - | - | The top-level model group. The flight model consists of an atmospheric model and the rocket assembly group |
| AtmoComponent | ExplicitComponent | `altitude` | `temperature`<br>`pressure`<br>`density`<br>`speed_of_sound` | The atmospheric model that constitutes one pa |
